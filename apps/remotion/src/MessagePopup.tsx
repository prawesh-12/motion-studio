...
const D_CARD = 0; // card pops in first
const D_SENDER = 14;
const D_TIME = 20;
const D_BODY_START = 30;
const WORD_STAGGER = 4; // frames between words

// Card dimensions — larger than before
...
export const MessagePopup: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        <NotificationBanner frame={frame} fps={fps} />
      </div>
    </AbsoluteFill>
  );
};

function NotificationBanner({ frame, fps }: { frame: number; fps: number }) {
  const bodyWords = NOTIFICATION.body.split(" ");

  // Card-level pop entry
